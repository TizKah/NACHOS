...
Memory:\n\
  ...
  Memory size: %u bytes.\n",
      PAGE_SIZE, machine->GetNumPhysicalPages(), TLB_SIZE, 
      machine->GetNumPhysicalPages() * PAGE_SIZE);
#else
    printf("\n\
Memory:\n\
   Page size: %u bytes.\n\
   ...
   Number of TLB entries: %u.\n\
   Memory size: %u bytes.\n",
      PAGE_SIZE, DEFAULT_NUM_PHYS_PAGES, TLB_SIZE, 
      DEFAULT_NUM_PHYS_PAGES * PAGE_SIZE);
